...
inline void showAssociatedProcessor(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    ...
    dbus::utility::getAssociatedSubTreePaths(
        ...
        [asyncResp,
         pcieSlotPath](const boost::system::error_code& ec,
            ...
            item["DBUS-CpuPath"] = endpoints[0];

            nlohmann::json::array_t processorArray = nlohmann::json::array();
            processorArray.emplace_back(std::move(item));

            asyncResp->res.jsonValue["Links"]["Oem"]["IBM"]["PCIeSlot"]
                                    ["Processors@odata.count"] =
                processorArray.size();
            asyncResp->res
                .jsonValue["Links"]["Oem"]["IBM"]["PCIeSlot"]["Processors"] =
                std::move(processorArray);

            asyncResp->res
                .jsonValue["Links"]["Oem"]["IBM"]["PCIeSlot"]["DBUS-SlotPath"] =
                pcieSlotPath;
        });
}
    ...
    const dbus::utility::MapperEndPoints& chassisPaths)
{
    ...
    //
    showAssociatedProcessor(asyncResp, pcieDeviceSlot);
}
...
{
    asyncResp->res.jsonValue["DBUS-PCIePath"] = pcieDevicePath;
